...
bot.remove_webhook(drop_pending_updates=True)  # إزالة أي Webhook قديم
...
def start(message):
    ...
    if cursor.execute('SELECT user_id FROM users WHERE user_id=?', (user_id,)).fetchone() is None:
        ...
        conn.commit()
        bot.send_message(ADMIN_ID, f"دخول نفـرر جديد لبوتك 😎\n• الاسم😂: {message.from_user.first_name}\n• معرف💁: @{message.from_user.username or 'لا يوجد'}\n• الايدي🆔: {user_id}\n• عدد مشتركينك الابطال: {get_total_users()}")
    ...
    welcome_text = "✨ **اهلا بك في بوت الخدمات المجانية 🆓** ✨\nالبوت سيساعدك في زيادة تفاعل قناتك ✅.\n- 𝚍𝚎𝚟: @E2E12"
    bot.send_message(message.chat.id, welcome_text, parse_mode="Markdown", reply_markup=main_inline_menu())
...
def handle_query(call):
    ...
    if call.data == "my_account":
        ...
        vip_text = "VIP 🌟" if is_vip else "عادي"
        markup = types.InlineKeyboardMarkup()
        markup.add(
            types.InlineKeyboardButton("مشاركة البوت", url=f"https://t.me/share/url?url=@{bot.get_me().username}"),
            types.InlineKeyboardButton("اشترك VIP", url=f"https://t.me/{bot.get_me().username}?start=vip")
        )
        bot.send_message(call.message.chat.id, f"👤 حسابك:\n• ايدي: {user_id}\n• المشتركين في البوت: {get_total_users()}\n• نوع الحساب: {vip_text}", reply_markup=markup)
        return

    if call.data.startswith("ser_"):
        data = call.data.split("_")
        service_type = data[1]  # sub, view, react
        service_id = data[2]
