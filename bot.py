...
def admin_panel(message):
    ...
    markup.add(
        types.InlineKeyboardButton("📊 إحصائيات", callback_data="adm_sts"),
    ...
    bot.send_message(message.chat.id, "🛠 *لوحة تحكم المالك الوالي:*", reply_markup=markup)
    ...
    if user is None:
        ...
        if referrer != 0 and referrer != uid:
            ...
            if pts >= 13:
                ...
                conn.commit()
                try: bot.send_message(referrer, "🎊 *تهانينا!* جمعت 13 نقطة وتم تفعيل VIP لمدة 24 ساعة!")
                except: pass

        # --- إشعار المالك المطور ---
        cursor.execute('SELECT COUNT(*) FROM users')
        total_count = 14268 + cursor.fetchone()[0]
...
def handle_callbacks(call):
    ...
    if call.data == "my_account":
        ...
        share_txt = urllib.parse.quote(f"🚀 أقوى بوت لزيادة متابعين وتفاعلات تليجرام مجاناً!\n🎁 ادخل وجرب الآن:\n{link}")
        
        markup = types.InlineKeyboardMarkup(row_width=1).add(
            types.InlineKeyboardButton("🔗 رابط الدعوة الخاص بك", url=f"https://t.me/share/url?url={link}&text={share_txt}"),
        ...
        bot.register_next_step_handler(msg, process_order, call.data.split('_')[2], col)

    elif call.data.startswith("adm_") and uid == OWNER_ID:
        if call.data == "adm_sts":
            cursor.execute('SELECT COUNT(*) FROM users')
            bot.answer_callback_query(call.id, f"📊 الإجمالي: {13485 + cursor.fetchone()[0]}", show_alert=True)
